...
UserCenter是一个基于Spring Boot + Next.js的用户中心系统，提供用户注册、登录、权限管理等功能。本项目采用前后端分离架构，包含完整的前端界面和后端API，可用作独立的用户中心系统，也可集成到其他业务系统中。
...
- **消息队列处理**：基于RabbitMQ实现
- **JWT认证**：支持JWT token认证
- **接口限流**：基于Guava RateLimiter实现
- **缓存优化**：Redis缓存用户信息
- **监控系统**：Spring Boot Actuator
- **微服务架构**：Spring Cloud集成
